...
import data.scripts.weapons.proj.espc_SemiRiftTrail;
...
public class espc_SemiRiftEffect implements OnFireEffectPlugin, EveryFrameWeaponEffectPlugin, WeaponEffectPluginWithInit {
		...
		if (weapon.getChargeLevel() > 0f && weapon.getCooldownRemaining() <= 0f) {
			...
			if (weapon.getChargeLevel() < 0.7f) {
				Global.getSoundPlayer().playLoop(
					"espc_riftchargeloop",
					weapon,
					1.0f - weapon.getChargeLevel() * 0.2f,
					weapon.getChargeLevel() * 4f,
					weapon.getLocation(),
					weapon.getShip().getVelocity()
				);
				...
				if (weapon.getChargeLevel() < 0.9f) {
					...
					if (dist < 1500f) {
						Global.getSoundPlayer().applyLowPassFilter(
								1f - (weapon.getChargeLevel() - 0.8f) * 0.2f * (1500f - dist)/1500f,
								1f - (weapon.getChargeLevel() - 0.9f) * 2f * (1500f - dist)/1500f
							);
					}
				}
				Global.getSoundPlayer().playLoop(
